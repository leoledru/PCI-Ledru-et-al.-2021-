function [out] = ecoevo_model_beta_poisson(tmax,total,host,symbiont,host_d,symbiont_d,space_size,...
...
%     new version : Poisson Distribution
    ...
    if sum(global_disp_s)+sum(local_disp_s)>0
        ...
        index = [repelem(symbiont,global_disp_s);repelem(symbiont,local_disp_s)]; % allow to find the correspondance between symbiont and localisation of colonization
        ...
        for ii = 1:length(colonization)
            if sum(disp==colonization(ii))>1 % several offsprings at the same position : uniform lottery
                ...
                symbiont(colonization(ii)) = index(disp==colonization(ii)); % if only one offspring -> colonization
